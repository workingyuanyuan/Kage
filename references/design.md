...
以下是 Kage 的視覺與操作基準。調整相關元件時使用；材質與色彩見 `material.md`。
...
模板以 `--serif`、`--sans`、`--mono` 共用字體堆疊，修改共用定義時依 [production.md](production.md)「模板庫維護」同步 BASE。
...
行內強調可用細下劃線與有語意的 `<strong>`。小字維持 `--color-text-on-dark`；橘棕色文字的字級與位置限制見 `material.md`「2.4 對比與可存取性」。
...
需要中英雙層標示時，主導覽、分類、章節標題採中文主標 + 英文副標，形成主從：
...
圖說依用途提供識別資訊、觀察或有依據的判斷，見 [writing.md](writing.md)「圖解」。
...
- **說明沿用 `[需要資料：…]` 記法，且必須是單一文字節點。** `kage placeholders` 用 `/\[需要資料[：:][^\]]{1,80}\]/` 辨識缺口，說明超過 80 字元或拆進 `<b>` 等子元素會漏報。
...
- **大螢幕等比放大**：簡報的每個 `clamp()` 在 viewport 1200px 就頂到上限，之後版面完全停止縮放 —— 實測 1920 與 3840 的算圖逐值相同（標題 72px、內文 18px、內容欄 1200px），4K 上等於把 1280 的版面原尺寸畫進四倍面積，內容欄只剩畫面寬的 31%。簡報是**填滿畫面的表面**，不是有固定行寬的閱讀文件，所以超過設計基準寬就要等比長回去。基準取 **1280**（`clamp` 在那裡到頂，且內容欄佔畫面 93.8%），倍率 `= viewport ÷ 1280`，以 0.25 為級距用 `min-width` 媒體查詢掛在 `.deck` 上，3840 剛好 3×。

  縮放走 `zoom` 而非逐條改 `font-size`：px 值散在字級、間距、微型字與按鈕四處，逐條改必然漏掉（實測漏改的眉標與頁腳會脫離層級），且 `--space-*` 屬於 BASE，動它會波及其餘七種文件。`.bg-fixed` 與 `.deck-ui` 都是 `.deck` 的**兄弟節點**，所以漏光與顆粒維持 1:1、操作列不跟著放大 —— 這正是使用者用瀏覽器整頁縮放時得不到的，那會連 200×200 的顆粒圖一起拉伸而糊掉。`zoom` 內的 `100dvh` 會先對未縮放的視窗求值再被放大，故頁高須同步除回倍率；媒體查詢只掛 `screen`，列印維持一頁一張不套縮放。
