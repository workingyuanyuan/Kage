...
以**單一畫面大小**的截圖為單位檢查。捲動文件建議在頂端、中段、底端各取一張；第 2 條的計算門檻是**兩張**，三張是取樣建議而非硬性下限（`shot --frames` 會一次切出整份文件的每一框）。
...
| 7 | chroma ≥ 12/255 的像素中，色相落在 18–30° 或 180–192° 之外的比例 < 1%（母體不足時不判定，見下） |
...
第 7 條的門檻是**絕對彩度**（`max−min ≥ 12/255`），不是相對飽和度。底色 luma 只有 13 上下，`r=20 g=13 b=11` 這種像素的 `d/mx` 高達 0.45 看似高飽和，實際 chroma 只有 9/255，而 8 bit 量化的 ±1 就能讓色相跳好幾度 —— 用相對門檻會把整片捨入雜訊判成違規。

第 7 條同時有**母體門檻**：chroma ≥ 12/255 的像素不足全圖 2% 時，這條標為無定義而不判定。它算的是「有彩像素裡有幾成越界」，而有彩像素的數量隨頁型與寬度差兩個數量級 —— 1280 的 C 級約 147,000 個，375 的 C 級只剩約 400 個（手機斷點把 `--leak-a` 降到 6%，漏光水平半徑 `min(55vw, 900px)` 在 375 下只有 206px）。分母一塌，三十幾個顆粒尖端的離群像素就能把比例推到 9%，但它們佔全圖僅 0.01%。真有色偏時大片像素帶彩，分母自然回到門檻之上，這條照樣會報紅。

驗收由 `node scripts/kage.mjs bg <截圖.png…>` 執行，吃截圖 PNG 輸出八項指標。**要餵 `shot --bg-only` 的背景層截圖**：含內容的畫面裡，紙張表面與米白正文會主宰明度統計，量到的是前景。
