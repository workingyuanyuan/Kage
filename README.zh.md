...
指定 Kage 或暗色類比風格的 HTML 排版，並說明文件用途。代理依需求選擇模板、語言與篇幅；缺少會影響結果的重要資訊時再提問。
...
**內容依材料編排。** 區塊與案例數量配合已有證據調整。必要素材尚缺時，在製作中的文件標示 `[需要資料：…]`，需要你提供資訊時提出明確問題。

**交件時你會拿到**：交付檔案連結、相關驗證摘要，以及需要處理的缺口或阻礙。文件呈現讀者所需的最終內容。

**回饋就說想改善的效果。** 代理依內容與可見版面調整；需要你的視覺偏好時，可透過具體問題或對照樣張確認。

### 交付前會跑的檢查

檢查依工作範圍選擇。成品確認內容與資產，新版面通常檢視桌面與手機畫面；材質變更使用背景像素檢查，模板、token 或 Schema 維護使用專案檢查。命令與適用情境見 [production.md](references/production.md)。
...
**模板容量可檢查。** 每種類型有一份 JSON Schema，描述既有結構的欄位與容量；保留該結構或整理結構化稿件時可使用。自訂文件依使用者內容調整並檢查實際版面，字串長度以 Unicode 字元計算。
